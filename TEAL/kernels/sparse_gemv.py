...
def splitk_sparse_gemv_kernel(
    ...
    # now compute the block that each program wixl go through
    ...
    if BATCHSIZE == 1:
        ...
        nonzero_counter = None
        if isinstance(nonzero_counter, torch.Tensor) and nonzero_counter.numel() > 0:
            nz = a != 0  # bool mask
            nz_int = nz.to(tl.int32)
            nz_count = tl.sum(nz_int)
            # atomic add to host tensor (global mem)
            tl.atomic_add(nonzero_counter, 0, nz_count, mask=tl.full((1,), True, tl.int1))
...
def qkv_kernel_logging(
    ...
    if BATCHSIZE == 1:
        ...
        nz_count = tl.sum(nz_int)
        # atomic add to host tensor (global mem)
        mask = tl.full((), True, tl.int1)  # shape: (), 정확히 value와 match
        tl.atomic_add(nonzero_counter, nz_count, mask=mask)
...
def qkv_gemv(
    ...
    nonzero_counter = torch.zeros(1, dtype=torch.int32, device=x.device)
    if eval_sparsity:
        qkv_kernel_logging[grid](
            output,
            weight,
            x,
            threshold_q, threshold_k, threshold_v,
        ...
        if x.dtype is not output.dtype:
            ...
            return output.to(dtype=x.dtype), nonzero_counter.to(dtype=x.dtype)
        
        return output, nonzero_counter
    else:
        qkv_kernel[grid](
            output,
            weight,
...
class SparseGEMV(BaseKernel):
    ...
    def forward(
        ...
        # if hidden_states.shape[1] == 1:
            # output, nonzero_counter = splitk_sparse_gemv(hidden_states, weights, threshold, sparsity_bin)
        # else:
            # output = 
        # return output
...
class SparseQKVGEMV(BaseKernel):
    ...
    def forward(
        ...
        if x.shape[1]==1:
            output, nonzero_counter = qkv_gemv(x, weight, threshold_q, threshold_k, threshold_v, sparsity_bin, kv_size)
        else:
