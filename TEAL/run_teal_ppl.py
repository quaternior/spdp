...
print("*******************************************")
print(optimal_s2)
wandb.init(project="teal-monitor", name=f"wanda_run")
# 5% 간격으로 s1을 생성 (0% 및 final_s 제외)
# for final_s in range (5, 96, 5):
results = []
for final_s in optimal_s2:
    ...
    cmd_pruning_2 = f"conda run -n {env_pruning_2} bash $SPDP_HOME/TEAL/scripts/ppl_test_teal.bash {final_s}"
...
csv_path = f"results/teal_corresponding_ppl_results.csv"
