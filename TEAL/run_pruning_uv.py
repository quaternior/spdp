...
for final_s in range (5, 91, 5):
    ...
    for s1 in range(5, int(final_s * 100), 5):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        msg = f"\n[INFO {timestamp}] Current sparsity: s1={s1_float:.4f}, s2={s2_float:.4f}"
        ...
        if not os.path.exists(model_name):
            print(f"[INFO {timestamp}] wanda({method}) based wanda.sh executing")
            cmd_pruning_1 = f"$SPDP_HOME/wanda/wanda.sh {final_s} {s1_float} {method} {0}"
            ...
            if process.returncode != 0:
                print(f"[ERROR {timestamp}] pruning_1.sh Execution failed (s1={s1_float})")
                continue
        else:
            print(f"[INFO {timestamp}] wanda({method}) 생략")
        # pruning_2.sh 실행 (teal 환경)
        print(f"[INFO {timestamp}] teal based grabnppl.bash 실행 중...")
        cmd_pruning_2 = f"$SPDP_HOME/TEAL/scripts/grabnppl.bash {final_s} {s1_float} {s2_float} {method} {gpu_idx} {eval_sparsity}"
        ...
        process.wait()
        if process.returncode != 0:
            print(f"[ERROR {timestamp}] pruning_2.sh execution failed (s2={s2_float})")
            continue
        ...
        if match:
            ...
            print(f"[WARNING {timestamp}] Cannot find uniform PPL (s1={s1_float})")
    ...
    print(f"[INFO {timestamp}] Saved CSV at {csv_path}")
    wandb.save(csv_path)
