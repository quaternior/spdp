...
class ModelArgs:
    ...
    def from_name(cls, name: str):
        if name in transformer_configs:
            print("direct from name")
            return cls(**transformer_configs[name])
        # fuzzy search
        print("fuzzy search from name ", name)
...
def _new_attn_forward(self, x: Tensor, freqs_cis: Tensor, mask: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
    ...
    output, nonzero_counter = self.gemv1(x, self.wqkv.weight, self.thresh_q, self.thresh_k, self.thresh_v, self.sparsity_bin, kv_size) # prefill logic taken care of in gemv
    q, k, v = output.split([self.dim, kv_size, kv_size], dim=-1)
    if nonzero_counter is not None: # Is not prefill
        sparsity_ratio = 1.0 - (nonzero_counter.item() / self.wqkv.weight.numel())
        print(f"sparsity_ratio: {sparsity_ratio:.4f}")
