...
class Distribution:
    def __init__(self, file_path, hidden_type):
        ...
        # print(f"[DEBUG]Load histogram from {self.file_path}/histograms.pt. bin_centers: {self.bin_centers}, counts: {self.counts}")
    ...
    def icdf(self, q):
        ...
        if idx == 0:
            # print(f"[DEBUG]Returned first case, q={q}, self.total_count={self.total_count}")
            return self.bin_centers[0]
        elif idx == len(self.bin_centers):
            # print(f"[DEBUG]Returned second case, q={q}, self.total_count={self.total_count}")
            ...
            # print(f"[DEBUG]q={q}, fraction={fraction}, lower_value={lower_value}, upper_value={upper_value}")
...
def get_sparse_model(model_name, device, histogram_path, eval_sparsity, **kwargs):
    ...
    if device == 'auto':
        # multi gpu
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.float16, device_map="auto", attn_implementation="flash_attention_2", histogram_path=histogram_path, eval_sparsity=eval_sparsity, **kwargs)
    else:
        return SparseModel.from_pretrained(model_name, torch_dtype=torch.float16, device_map=device, attn_implementation="flash_attention_2", histogram_path=histogram_path, eval_sparsity=eval_sparsity, **kwargs)
