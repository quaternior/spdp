...
class LlamaMLP(nn.Module):
    def __init__(self, config, k_factor):
        ...
        assert self.mode in ['gen', 'class', 'ppl']
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            if self.mode == 'gen':
                ...
                if x.shape[1] > 1:
                    int_states = self.act_fn(self.gate_proj(x[:, 0:x.shape[1]//2])) * self.up_proj(x[:, 0:x.shape[1]//2])
                    ...
                    down_proj_pre = self.down_proj(int_states)

                    if k_factor == 0.0:
                        down_proj_post = 0 * x[:, x.shape[1]//2:]
                    else:
                        down_proj_post =self.down_proj_reduced(self.act_fn(self.gate_proj_reduced(x[:, x.shape[1]//2:])) * self.up_proj_reduced(x[:, x.shape[1]//2:]))
                    
                    down_proj = torch.cat([down_proj_pre, down_proj_post], dim=1)
